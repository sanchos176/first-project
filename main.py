...
pygame.display.set_caption("Tank simulator")
#shoot_pygame = pygame.mixer.Sound("shoot.wav.mp3")
#move_forward_sound = pygame.mixer.Sound("move.forward.mp3")
#move_gun = pygame.mixer.Sound("move_forward.mp3")
...
class Tank(pygame.sprite.Sprite):
    def __init__(self, x, y):
        super().__init__()
        self.original_tank_image = pygame.image.load("Hull_01.png").convert_alpha()
    ...
    def update(self):
        ...
        if keys[pygame.K_UP]:
            self.speed = 3
            #move_forward.sound.play()
        elif keys[pygame.K_DOWN]:
            self.speed = -2
        else:
            self.speed = 0
            #move_forward.sound.stop()
        ...
        self.rect = self.image.get_rect(center=(self.rect, self.rect))
...
class Gun(pygame.sprite.Sprite):
    def __init__(self, tank):
        super().__init__()
        self.original_gun_image = pygame.image.load("Gun_06.png").convert_alpha()
    ...
    def update(self):
        ...
        if keys[pygame.K_SPACE] and self.can_shoot:
         self.gun_back_start_time = pygame.time.get_ticks()
         bullet_angle = self.angle
         bullet_x = self.rect.centerx + (self.gun_lenght + self.tip_offset) * math.cos(math.radians(bullet_angle))
         bullet_y = self.rect.centery + (self.gun_lenght + self.tip_offset) * math.sin(math.radians(bullet_angle))
        ...
        if elapsed_time <= self.cooldown_duration:
            ...
            self.gun_lenght = self.original_gun_length - 5 * progress
        else:
            self.gun_lenght = self.original_gun_length


        angle_rad = math.radians(self.angle)
        gun_end_x = self.tank.rect.centerx + (self.gun_lenght + self.tip_offset) * math.cos(angle_rad)
        gun_end_y = self.tank.rect.centery + (self.gun_lenght + self.tip_offset) * math.csin(angle_rad)
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, angle):
        super().__init__()
        self.original_bullet_image = pygame.image.load("Medium_Shell.png").convert_alpha()
        ...
        dx = (gun.gun_lenght+ gun.tip_offset - 32) * math.cos(angle_rad)
        dy = (gun.gun_lenght + gun.tip_offset - 32) * math.sin(angle_rad)
...
while running:
    ...
    if gun.can_shoot and keys[pygame.K_SPACE]:
       ...
       bullet_x = gun.rect.centerx + (gun.gun_lenght + gun.tip_offset) * math.cos(math.radians(bullet_angle))
       bullet_y = gun.rect.centerx + (gun.gun_lenght + gun.tip_offset) * math.cos(math.radians(bullet_angle))
       bullet = Bullet(bullet_x, bullet_y, bullet_angle)
       all_sprites.add(bullet)
